...
public class NoticeService {
	...
	public List<NoticeDto> getAll() {
		return repo.findAll(org.springframework.data.domain.Sort.by("regDt").descending()).stream().map(n -> {
			NoticeDto dto = NoticeDto.fromEntity(n);
			List<FileInfoDTO> files = fileService.getFileList(FileConstants.RefType.NOTICE, n.getNoticeId(),
	...
	public Map<String, Object> getNoticesWithPagination(NoticeSearchDTO searchDTO) {
		...
		List<NoticeDto> list = noticePage.getContent().stream().map(n -> {
			NoticeDto dto = NoticeDto.fromEntity(n);
			List<FileInfoDTO> files = fileService.getFileList(FileConstants.RefType.NOTICE, n.getNoticeId(),
					FileConstants.Category.ATTACH);
			dto.setFiles(files);
	...
	public NoticeDto get(String noticeId) {
		return repo.findById(noticeId).map(n -> {
	...
	public String createWithFiles(NoticeDto dto, List<MultipartFile> files) {
		...
		if (files != null && !files.isEmpty()) {
			fileService.uploadFiles(files, FileConstants.RefType.NOTICE, id, FileConstants.Category.ATTACH,
					dto.getRegUserId());
		}
	...
	public void update(String noticeId, NoticeDto dto) {
		Notice n = repo.findById(noticeId).orElseThrow(() -> new IllegalArgumentException("공지사항을 찾을 수 없습니다."));
		n.setTitle(dto.getTitle());
		n.setContent(dto.getContent());
	}
	...
	public void updateWithFiles(String noticeId, NoticeDto dto, List<MultipartFile> files) {
		...
		// 기존 첨부파일 논리삭제
		fileService.deleteFilesByRef(FileConstants.RefType.NOTICE, noticeId, dto.getRegUserId());

		// 새로운 파일 업로드
		if (files != null && !files.isEmpty()) {
			fileService.uploadFiles(files, FileConstants.RefType.NOTICE, noticeId, FileConstants.Category.ATTACH,
					dto.getRegUserId());
		}
	}
	...
	public List<FileUploadResponseDTO> uploadNoticeFiles(String noticeId, List<MultipartFile> files, String userId) {
		return fileService.uploadFiles(files, FileConstants.RefType.NOTICE, noticeId, FileConstants.Category.ATTACH,
				userId);
	}
	...
	public void deleteNoticeFile(String noticeId, Long fileId, String userId) {
		...
		if (info == null || !FileConstants.RefType.NOTICE.equals(info.getRefType())
				|| !noticeId.equals(info.getRefId())) {
	...
	public FileInfoDTO getNoticeFileInfo(String noticeId, Long fileId) {
		...
		return fileService.getFileList(FileConstants.RefType.NOTICE, noticeId, FileConstants.Category.ATTACH).stream()
				.filter(f -> f.getFileId().equals(fileId)).findFirst().orElse(null);
	}
	...
	public void delete(String noticeId, String userId) {
		...
		// 첨부파일 삭제
		fileService.deleteFilesByRef(FileConstants.RefType.NOTICE, noticeId, userId);
